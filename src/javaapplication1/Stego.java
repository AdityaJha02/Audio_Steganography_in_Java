...
class Stego {
  ...
  //Constructor 1
  public Stego(String sndFile, String ptFile, String oFile, String embT){
    //pbeKeySpec=pwd;
   ...
   System.out.println("Pwd " +pwd);*/
    //password=pwd;
    //embText = embT.toCharArray();
    password = embT.toCharArray();
    System.out.println("Inside Constructor: "+password+"\nembT Val:  "+embT);
    outFile=oFile;
    readSND(sndFile);
  ...
  public void encode(){
    ...
    System.out.println("ENCODE@Audio Bytes: "+audioBytes.toString());
  ...
  public boolean decode(){
    ...
    System.out.println("hereeee"+buff);
    //System.out.println("Decrypted Message: "+AESImpl.decrypt(new String(buff, StandardCharsets.UTF_8),password.toString()));

          System.out.println("Decrypted message : "+SelectDataToEmbed.getEmbeddedText());
      ...
      //outfile.write(clearbuff);
          outfile.write(SelectDataToEmbed.getEmbeddedText().getBytes());

          outfile.close();
    }catch(Exception e){
          System.out.println("Decrypted message11111"+SelectDataToEmbed.getEmbeddedText());
  ...
  private void readSND(String sndf) {
    ...
    System.out.println("Insdie readSND method "+sndfile);
    try {
  ...
  private boolean possible(String pt) {
   ...
   try{
    //encrypt(pt);
    //Converting cryptic char array to byte array
    
    cipherbuff = Charset.forName("UTF-8").encode(CharBuffer.wrap(password.toString())).array();
    System.out.println("Cipher Buff :"+cipherbuff+"\n Embedded Text: "+ password.toString());
  ...
  //---------------------------------------------------------
   ...
   //-------------------------------------------------
   private void encrypt(String ptr){
    // PBEKeySpec pbeKeySpec;
