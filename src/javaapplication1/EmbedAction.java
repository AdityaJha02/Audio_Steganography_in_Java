public class EmbedAction  implements WizardButtonListener
 {
    ...
    public EmbedAction(DHMain m) {
        ...
        wf.addWizPanel(sdte);
        eoo = new EnterOtherOptions("4. Enter Encryption Key");
        wf.addWizPanel(eoo);
    ...
    public void buttonClicked(WizardButtonEvent wbe) {
        if(wbe.getButtonType() == WizardButtonListener.NEXT  && wbe.getCard() == sdte){
            String str = sdte.getEmbeddedText();
            eoo.setEmbeddedText(str);   
        }
        else if( wbe.getButtonType() == WizardButtonListener.NEXT  && wbe.getCard() == eoo ){
            ...
            if( sdte.isTextOrFile() ){
                System.out.println(eoo.getEmbeddedText());
                options.setEmbedText( eoo.getEmbeddedText() );
